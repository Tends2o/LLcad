...
- `reports/llm-eval.json` belegt sechs tatsächliche Modellturns im installierten Codex-Host für den dort angegebenen Build: Nut, Durchmesser, Mehrdeutigkeit, einzelne Instanz, bestehende lokale Feldkorrektur und Tasche in einem gedrehten Bauteil. Der Grader prüft die gespeicherte Geometrie und Werkzeugaufrufe; Modellbehauptungen allein zählen nicht als Erfolg. Die früheren Konfigurationsfehler sind in `reports/llm-eval-initial.json` als fehlgeschlagener Versuch dokumentiert.
...
- `reports/rebuild-existing-plan.json` und `rebuild-existing-applied.json` dokumentieren die tatsächlich neu berechneten vorhandenen Modelle, die unveränderten Konstruktionen, den Messvergleich, Prüfdigests und den Erhalt ihrer alten Revisionen.
