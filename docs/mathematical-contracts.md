...
Oberflächenzellen werden mit einer Lipschitz-Schranke oder wahlweise mit rigoroser Intervallarithmetik (`extraction.pruning: "interval"`) räumlich ausgeschlossen. Verbleibende Blattzellen werden mit Marching Tetrahedra oder mit Dual Contouring und QEF-Punktwahl (`extraction.method`) extrahiert. Die gewählte Zellweite beweist keine erhaltene Subzelltopologie. Die Ausgabe trägt `preview_only` und `certified_bound: null`.

Zertifikate zwischen zwei Feldern (`surface_deviation`), blendfreie Passregionen (`blend_free_region`), zertifizierte Wertebereiche und die Regularitätsprüfung der impliziten Krümmung (`cad_measure metric=curvature` mit `point`) verwenden die Intervallauswertung mit gerichteter Rundung; siehe [intervals-and-certificates.md](intervals-and-certificates.md). Abgetastete Gitter (`sampled_grid`) sind interpolierende natürliche kubische B-Splines mit gemessener Lipschitz-Schranke; siehe [imports.md](imports.md).

## Volumetrische Dateien und Wiederverwendung

OpenVDB 10 exportiert maximal 125.000 Float32-Samples eines einzelnen analytischen Feldes. Ein explizites Band begrenzt gespeicherte Werte; Metadaten unterscheiden ursprüngliche Feldsemantik, gespeicherte Samples, Domäne und tatsächliches Rasterende. Jeder gespeicherte Rasterwert wird nach dem Dateiexport erneut gelesen und verglichen. Dies zertifiziert weder kontinuierliche Distanz noch Topologie unterhalb der Rasterweite. Der Import eines achsparallelen skalaren Grids als Gitterfeld (bis zwei Millionen aktive Voxel) ist registriert und in [imports.md](imports.md) beschrieben.

Ein privater räumlicher Samplecache ist an Mandant, Eigentümer, Modell, Feature und Registry gebunden. Gleiche Punkte können zwischen Auflösungen wiederverwendet werden; außerhalb der kompakten Stütze eines lokalen Edits wird der unveränderte Ausdruck erkannt. Innerhalb inverser Deformationen wird keine unzulässige Gleichheit des Quellpunkts angenommen. Pro Feature bleiben maximal 160.000 Samples erhalten. Jeder native Auftrag läuft in einem eigenen isolierten Einweg-Prozess; ein vorgewärmter Prozess hat lediglich die Bibliotheken importiert und wartet ohne Auftragsdaten, sodass kein veränderlicher Workerzustand in den nächsten Auftrag gelangt.
...
`thread` baut ein echtes Helixgewinde mit dreieckigem oder trapezförmigem Profil, rechts- oder linksgängig, außen oder als Schnitt in einen vorhandenen Körper, mit optionalem Auslauf und Selbstdurchdringungsprüfung. Registriert sind `custom` und das ISO-Grundprofil `iso_metric_basic` (Grundgeometrie ohne Toleranzklassen); siehe [threads.md](threads.md). Höchstens 32 Windungen sind erlaubt. `offset_solid`, Loft-Kompatibilitätsprüfung, geregelte Lofts, Sweeps mit rotationsminimierenden Rahmen, Twist, Endskalierung und Selbstkontaktprüfung sowie die Netzoperatoren aus [mesh-operators.md](mesh-operators.md) sind ebenfalls registriert. Bézier-Kurven und Tensorproduktflächen besitzen in `workers/cad-occt/bezier.py` eine exakte rationale Bernstein-Auswertung mit Ableitungen, de-Casteljau-Unterteilung und Graderhöhung, die in `tests/geometry/test_bezier.py` gegen den nativen Kernel geprüft wird.

## Nachweisgrenzen

`checks_passed_within_profile` bedeutet ausschließlich, dass die tatsächlich registrierten Prüfungen des Profils bestanden sind. `null` ist kein Fehler von null. Wandstärke, Freigang entlang einer Bewegung, Chamfer-/Hausdorff-Proben und die Regeln des Profils `manufacturing_candidate` sind ausdrücklich abgetastete Aussagen (`sampled`); exakte Kernelextrema und boolesche Volumina tragen `exact_for_declared_domain`; Zertifikate tragen `bounded` ([measures.md](measures.md)). Statik und eine Fertigungszertifizierung gehören nicht zu den freigegebenen Profilen dieser Version. Das Fehlerbudget (`error_budget`) addiert nur kompatible zertifizierte Stufen und weist nicht summierbare Nachweise getrennt aus.
